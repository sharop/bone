...
func (b *BadgerStore) FirstIndex() (uint64, error) {
	...
	err := b.conn.View(func(txn *badger.Txn) error {
		...
		if it.ValidForPrefix(prefixLogs) {
			value = common.BytesToUint64(it.Item().Key()[1:])
		}
		return nil
	})
	if err != nil {
...
func (b *BadgerStore) GetLog(index uint64, log *raft.Log) error {
	return b.conn.View(func(txn *badger.Txn) error {
		item, err := txn.Get(append(prefixLogs, common.Uint64ToBytes(index)...))
		if err != nil {
			switch err {
		...
		return common.DecodeMsgPack(val, log)
	})
}

// StoreLog stores a single raft log.
func (b *BadgerStore) StoreLog(log *raft.Log) error {
	val, err := common.EncodeMsgPack(log)
	if err != nil {
		return err
	}
	return b.conn.Update(func(txn *badger.Txn) error {
		return txn.Set(append(prefixLogs, common.Uint64ToBytes(log.Index)...), val.Bytes())
	})
}
...
func (b *BadgerStore) StoreLogs(logs []*raft.Log) error {
	...
	for i, log := range logs {
		key := append(prefixLogs, common.Uint64ToBytes(log.Index)...)
		val, err := common.EncodeMsgPack(log)
		if err != nil {
			return err
		}
...
func (b *BadgerStore) DeleteRange(min, max uint64) error {
	...
	start := append(prefixLogs, common.Uint64ToBytes(min)...)
	for it.Seek(start); it.Valid(); it.Next() {
		key := make([]byte, 9)
		it.Item().KeyCopy(key)
		// Handle out-of-range log index
		if common.BytesToUint64(key[1:]) > max {
			break
		}
		...
		if err := txn.Delete(key); err != nil {
			if err == badger.ErrTxnTooBig {
				...
				return b.DeleteRange(common.BytesToUint64(key[1:]), max)
			}
			return err
		}
	}
...
func (b *BadgerStore) SetUint64(key []byte, val uint64) error {
	return b.Set(key, common.Uint64ToBytes(val))
}
